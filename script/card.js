...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const locations = [
    ...
    // أضف المزيد من المواقع هنا <br> Matière organique (MO%) :  <br></br>Phosphore
   ];
    ...
    L.marker([location.lat, location.lng])
      .bindPopup(`
          <div>
              ...
              <button style="justify-content: center;" class="redirectButton" data-id="${location.id}" data-lat="${location.lat}" data-lng="${location.lng}">Voir plus</button>
 ...
 // Ajouter un écouteur pour les boutons avec la classe "redirectButton"
document.addEventListener('click', function(event) {
  if (event.target.classList.contains('redirectButton')) {
      ...
      const url = `/card.html?id=${id}&lat=${lat}&lng=${lng}`;
